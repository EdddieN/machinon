...
Write standard Raspian Lite image to SD card.
With SD card still in PC card reader, create new empty file ssh in root directory (/boot partition) to enable SSH login.
...
Disable bluetooth service from starting (prevents service startup errors):
...
Connect the Pi to the Machinon board with the GPIO ribbon cable and power up. Wait for the Pi to boot, then find its IP and log in via SSH (use PuTTY or similar SSH client)
Run raspi-config and:
Enable SPI
Enable I2C
Enable Serial (but disable login shell over serial) (should already be done by the previous edits to config.txt and cmdline.txt)
Expand filesystem to fill card
Change GPU memory to 16 MB (not essential)
...
Change the default password, or optionally add a new user to run everything as instead of "pi". See https://mattwilcox.net/web-development/setting-up-a-secure-home-web-server-with-raspberry-pi for ideas.
Install any other desired updates, packages, etc. For Domoticz suggestions see https://www.domoticz.com/wiki/Raspberry_Pi#Raspberry_Pi_additional_software_installations
Domoticz Setup
...
Install latest Domoticz release using the command below. See official install guide at https://www.domoticz.com/wiki/Raspberry_Pi
curl -L install.domoticz.com | sudo bash
(choose HTTP port=8080 and HTTPS port=4443 when prompted)
Optional: Change the service startup mode to use systemd (on Raspian Stretch) and change user that Domoticz runs as (default is root if running under init.d ???). See https://www.domoticz.com/wiki/Linux for info.
...
Uncomment (remove # from) lines:
#User=pi
#Group=users
Change -www and -sslwww ports if required (0 to disable)
...
Can also run from console for testing and interactive output:
cd /path/to/domoticz
./domoticz
Add Hardware on Domoticz
Add MySensors USB Gateway new hardware under ‘Hardware’ menu
Call is machinon IO
Select Serial port: serial0
Add ModBus (if modbus plugin active) and select serial485
Edit machinon I/Os with names
...
Install NGINX (and php-fpm if required. On Raspian Stretch this will install PHP7.0)
sudo apt install nginx php-fpm
For HTTPS access on NGINX, either install your own certificate and specify this in the config file below, OR install the ‘snakeoil’ non-production certificate:
apt-get install ssl-cert
Create a new empty NGINX config file:
cd /etc/nginx/sites-available
sudo nano nginx-machinon.conf
Paste in the following content, then save/exit (Ctrl+X):
...
Disable the default config by deleting the "sites-enabled" symbolic link:
sudo rm /etc/nginx/sites-enabled/default
Create symbolic link to the config file so that NGINX uses it:
...
Clone the config forms PHP files and resources to the NGINX web pages directory:
cd /var/www/html
